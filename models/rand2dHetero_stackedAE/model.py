...
class denoiser(object):
    def __init__(self, sess, input_c_dim=1, batch_size=64, im_w=100, im_h=100):
        self.sess = sess
        ...
        self.Y_flat = tf.layers.flatten(self.Y) 

        self.hd1 = tf.layers.dense(self.X_flat, 5000, activation=tf.nn.relu, name="hidden1")
        self.hd2 = tf.layers.dense(self.hd1,    2500, activation=tf.nn.relu, name="hidden2")
        self.hd3 = tf.layers.dense(self.hd2,    5000, activation=tf.nn.relu, name="hidden3")

        self.reconX = tf.layers.dense(self.hd3, 10000, name="reconX")

        self.output = tf.reshape(self.reconX, [-1, 100, 100, input_c_dim])
        ...
        self.loss =  tf.reduce_mean(tf.square(self.reconX - self.Y_flat))
    ...
    def save(self, iter_num, ckpt_dir, model_name='stackedAE'):
        saver = tf.train.Saver()
