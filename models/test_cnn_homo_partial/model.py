...
def save_filter2mat(objName, objData, saveDir = 'mfiles'):
    ...
    print(savePath)
    sio.savemat(savePath, {objName:objData})
...
class denoiser(object):
    def __init__(self, sess, input_c_dim=1, batch_size=128):
        ...
        # start of model
        #---------------------------------------------------------------------#
        self.conv1 = tf.layers.conv2d(self.X, 6, 3, padding='same', name='conv1', activation=tf.nn.relu)
        self.conv2 = tf.layers.conv2d(self.conv1, 6, 3, padding='same', name='conv2', activation=tf.nn.relu)
        self.conv3 = tf.layers.conv2d(self.conv2, 6, 3, padding='same', name='conv3', activation=tf.nn.relu)
        self.conv4 = tf.layers.conv2d(self.conv3, 1, 3, padding='same', name='conv4')

        # deduct the noise from the input
        self.Y = self.X - self.conv4

        t_vars = tf.trainable_variables()
        self.conv1_kernel = [v for v in t_vars if 'conv1' in v.name  and 'kernel' in v.name][0] # 1st one in the list
        self.conv2_kernel = [v for v in t_vars if 'conv2' in v.name  and 'kernel' in v.name][0]
        self.conv3_kernel = [v for v in t_vars if 'conv3' in v.name  and 'kernel' in v.name][0]
        self.conv4_kernel = [v for v in t_vars if 'conv4' in v.name  and 'kernel' in v.name][0]
    ...
    def train(self, noisy_data, clean_data, batch_size, ckpt_dir, epoch, lr, eval_every_epoch=2):
        ...
        conv3_filter, conv4_filter = None, None
        ...
        conv1_filter = self.conv1_kernel.eval()
        conv2_filter = self.conv2_kernel.eval()
        conv3_filter = self.conv3_kernel.eval()
        conv4_filter = self.conv4_kernel.eval()

        objName,objData = 'conv1_filter', conv1_filter
        save_filter2mat(objName, objData)
        objName,objData = 'conv2_filter', conv2_filter
        save_filter2mat(objName, objData)
        objName,objData = 'conv3_filter', conv3_filter
        save_filter2mat(objName, objData)
        objName,objData = 'conv4_filter', conv4_filter
        save_filter2mat(objName, objData)
