...
rand_seed = randi([1 2^31-1], 1, N * repN);  % each clean image will need 10x1e8
	...
	for tid =1:N
        ...
        if ~exist(dir_phn_test, 'dir')  mkdir(dir_phn_test); end
				...
				for jj=1:repN
					...
					cfg.tend=5e-8;
					cfg.tstep=5e-8;
					cfg.seed = rand_seed((tid - 1)*N + jj); % each random seed will have different pattern 
				...
				data = data / float(repN); % avg


				%---------------------------
				% end of 10x 1e8 simulation 
				%---------------------------
...
% Generate new random seed for Monte Carlo simulation
rand_seed = randi([1 2^31-1], 1, N * 10);  % run 10x 1e8 for one clean image
