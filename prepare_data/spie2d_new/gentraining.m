function [cwdata, imdata]=gentraining(nphoton, imsize, maxobj, maxrepeat, randseed)
...
if(maxobj==0) % homogeneous case 
    ...
    for j=1:maxrepeat
        [cw, myimg]=rand_2d_mcx(nphoton, 1, imsize, myseed, mcxseed(j));
        cwdata(:,:,j)=cw;
        imdata(:,:,j)=myimg;
    end
else
    ...
    for i=1:maxobj
        for j=1:maxrepeat
            [cw, myimg]=rand_2d_mcx(nphoton, i+1, imsize, myseed(i), mcxseed((i-1)*maxrepeat+j));
            cwdata(:,:,j,i)=cw;
            imdata(:,:,j,i)=myimg;
        end
    end
