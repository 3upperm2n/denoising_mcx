...
def dncnn_unet(input, is_training=True, output_channels=1):
    ...
    with tf.variable_scope('unet-output'):
        ...
        output_unet = tc.layers.conv2d(up0e, 1, [1, 1], activation_fn=tf.nn.relu)
...
class denoiser(object):
    ...
    def plot(self, noisydata, ckpt_dir, outFile=''):
        """Test unet"""
        import scipy.io as sio
        ...
        input_unet_img, output_clean_image = self.sess.run([input_unet, self.Y],
                feed_dict={self.X: noisydata, self.is_training: False})

        print input_unet_img.shape
        print output_clean_image.shape
